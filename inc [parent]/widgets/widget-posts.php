		...
		?>
			...
			<?php if( $posts->have_posts() ) : ?>
				...
				<aside class="widget WPlookPosts">	
					<?php if ( $title != "") { /* Added for formatting reasons --SM20141123 */ ?>
						...
						<?php while( $posts->have_posts() ) : $posts->the_post(); ?>
							...
							<article class="cause-item">
								...
								<?php 
									...
									$current_post_year=$post_year;
									$post_year = substr(get_the_date($d, $the_post->post_parent), -4); /* Get the year it was posted */ 
									if($categories==6){ // 6 is the Featured Media Mention category and should be pinned. --SM20141124
										$post_year = "Featured";
									}
									if($current_post_year!=$post_year){
										echo '<p class="post_year">'.$post_year.'</p>';
									}
								?>
								...
								<?php 
									$parsed_url=parse_url($permalink = get_permalink($post->ID)); /* Parse the permalink to get the base URL for display --SM20141123 */ 
									if ( strpos($parsed_url["host"],'3halves') !== false ){
										$parsed_url = "Video";
									}else{
										$parsed_url = $parsed_url["host"];
									}
								?>
								<div class="short-description">
									<p><a href="<?php the_permalink(); ?>" class="<?php echo $class; ?>"><?php echo $parsed_url; ?></a></p>
								</div>
							</article>
